...
- **Platform Status Board**: Collapsible component on the login page showing active platform issues (INVESTIGATING, IN_PROGRESS, MONITORING, RESOLVED). Public API at `/api/platform-status` (no auth, no internal IDs). Admin CRUD at `/api/admin/platform-issues`. Admin management page at `/admin/platform-status`. Features status filtering, color-coded badges, active issue count, and last-checked timestamp.
- **User Submissions System**: Allows all logged-in users to report issues and submit suggestions from a "Feedback & Support" page (`/feedback`). Users can track their submissions and see admin responses. Admin management at `/admin/submissions` with filtering by type (Issue/Suggestion) and status (SUBMITTED, UNDER_REVIEW, IN_PROGRESS, COMPLETED, DECLINED). Admins can update status and respond to users.
- **Multi-Program Management**: Admin page at `/admin/programs` for managing mentorship programs and enrolling users. Super Admins can view all programs, see member counts, add/remove members with role assignment (ADMIN, MENTOR, MENTEE), bulk add users, change member roles, and filter/search members. Backend APIs at `/api/admin/programs` and `/api/admin/program-memberships`.
- **Profile Management System**: Comprehensive user profile editing for both users (/my-profile) and admins (/admin/users/:id/profile). Features tabbed interface with Core Info (personal/professional details), Role Selection (seeking_mentor/providing_mentorship/both), Mentee-specific fields (goals, preferred methods, duration), and Mentor-specific fields (experience, availability, skills). Supports SONSIEL membership status, fields of expertise, education level, certifications, and program expectations. Profile photo upload via Object Storage with camera overlay on avatar. Profile view page (/profile/:userId) displays full mentor/mentee profile data including career goals, mentorship history, skills, availability, and preferences. Photos served via `/api/profile-photo/:userId` endpoint.
